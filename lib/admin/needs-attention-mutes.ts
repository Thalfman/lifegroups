...
//
// This module is the thin server glue that loads the stored flag state and asks
// the pure resolver (lib/admin/feature-flags resolveMutedAttentionKeys) which
// dashboard categories are muted. It mirrors lib/admin/frozen-surface exactly:
// the flag state is read through the admin-readable admin_read_feature_flags()
// RPC rather than platform_config directly, because platform_config is
// Super-Admin-only by RLS but requireAdmin() admits ministry_admin too. The RPC
// returns only the feature_flags sub-object to both admin roles, so the mute
// resolves identically for the whole admin team's Home view — which is the
// point: ministry admins should not see the new ministry as behind either.
...
// non-admin caller (the RPC returns an empty flag map) shows every category
...
export async function getMutedAttentionKeys(): Promise<string[]> {
  ...
  return Array.from(resolveMutedAttentionKeys(decodeFeatureFlags(data)));
}
