...
function policyChunksFor(table: string): string[] {
  return allMig
    ...
    .slice(1)
    .filter((chunk) =>
      chunk.slice(0, chunk.indexOf(";")).includes(`on public.${table}`)
    );
}
...
function balancedAfter(haystack: string, marker: string, from = 0): string {
  ...
  for (let i = at; i < haystack.length; i += 1) {
    ...
    if (ch === "(") {
      ...
      depth -= 1;
      if (depth === 0)
        return haystack.slice(begin, i).replace(/\s+/g, " ").trim();
    }
  }
  return "";
}
...
describe("SC.4 boundary — only the creating ministry_admin can read (RLS predicate)", () => {
  it("across ALL migrations, each table's ONLY policy is the exact creator-scoped SELECT", () => {
    ...
    for (const table of TABLES) {
      ...
      expect(
        policies.length,
        `${table} must have exactly one policy (found ${policies.length})`
      ).toBe(1);
      const decl = policies[0].slice(0, policies[0].indexOf(";"));
      expect(decl, `${table} policy must be SELECT-only`).toContain(
    ...
    for (const table of TABLES) {
      for (const chunk of policyChunksFor(table)) {
        ...
        for (const call of calls) {
          expect(["public.auth_role(", "public.auth_profile_id("]).toContain(
            call
          );
        }
        for (const role of NON_CREATOR_ROLES) {
          expect(using, `policy must not admit ${role}`).not.toContain(
            `'${role}'`
          );
        }
      }
...
describe("SC.4 boundary — every writer is a ministry_admin-only SECURITY DEFINER RPC", () => {
  it("all SC.4 write functions gate on the ministry_admin role and pin search_path", () => {
    ...
    for (const fn of fns) {
      ...
      expect(
        body,
        `${fn} must scope/write created_by via the derived actor`
      ).toContain("created_by_profile_id");
      expect(body, `${fn} must use the derived actor in its writes`).toMatch(
        /\bv_actor\b/
      );
...
describe("SC.4 boundary — a stored row exposes ciphertext only, never plaintext", () => {
  it("no migration gives the note table a plaintext column (CREATE or ALTER)", () => {
    ...
    let from = 0;
    for (;;) {
      const at = allMig.indexOf(
        "alter table public.shepherd_care_private_notes",
        from
      );
      if (at === -1) break;
      ...
      if (stmt.includes("add column")) {
        expect(
          stmt,
          "no plaintext column may be added to the note table"
        ).not.toMatch(/\b(text|varchar|char|body|plaintext|note)\b/);
      }
      from = at + 1;
    }
  });
