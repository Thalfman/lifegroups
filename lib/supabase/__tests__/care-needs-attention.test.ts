...
describe("computeNeedsAttention — care status (#122)", () => {
  it("flags concern and needs_follow_up even with fresh contact", () => {
    expect(computeNeedsAttention(care({ current_status: "concern" }), TODAY)).toBe(true);
    expect(
  ...
  it("does not flag doing_well / needs_encouragement / inactive on status alone", () => {
    expect(computeNeedsAttention(care({ current_status: "doing_well" }), TODAY)).toBe(false);
    expect(
      computeNeedsAttention(care({ current_status: "needs_encouragement" }), TODAY),
    ).toBe(false);
    expect(computeNeedsAttention(care({ current_status: "inactive" }), TODAY)).toBe(false);
  });
