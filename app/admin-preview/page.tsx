...
export default async function AdminPreviewPage() {
  const { source, data, error } = await getAdminDashboardData();
  const totalPipeline = data.guestPipelineBreakdown.reduce((sum, row) => sum + row.count, 0);

  return (
    <AppShell
      title="Admin Dashboard Preview"
      subtitle="Ministry-level visibility composed from read-only Supabase queries with a safe demo fallback."
