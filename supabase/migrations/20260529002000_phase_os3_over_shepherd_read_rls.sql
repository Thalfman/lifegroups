...
--
-- Target eligibility (the covered profile must be an active leader/co_leader)
-- is enforced inside auth_over_shepherd_covers() as of phase_os6, so a
-- mis-created or stale assignment pointing at an above-ladder / deactivated
-- profile grants nothing here. The care-profile and care-interaction policies
-- below route through the same helper, so all three share that one fence.
