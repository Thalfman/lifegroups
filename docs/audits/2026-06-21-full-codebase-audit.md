...
A read-only audit of the Lifegroups admin operating system (Next.js 15 / React
...
| **P1 / High**     | 7     | Visible vocabulary drift against ADR 0025 (code strings still say "Lead"/"Leads"); a glossary line that contradicts ADR 0024; duplicate ADR numbers; the integration/RLS suite not gating on RLS-touching PRs; no end-to-end coverage of the core write flows. |
| **P2 / Medium**   | 11    | Maintainability (two large modules), review-only security seams worth a divergence test, coverage debt in hooks/observability, stale index lines.                                                                                                              |
| **P3 / Low**      | 8     | Cosmetic naming, missing ADR `Status:` headers, presentational components covered only indirectly.                                                                                                                                                             |
...
inclusion. Corrections made during verification are noted inline (e.g. one pass
cited a non-existent doc; another mislabelled "People" as "Groups").
...
| SEC-3 | P2  | `supabase/functions/invite-user/index.ts`, `redeem-invite/index.ts`                                                                                                                                        | The service-role edge functions already redact secrets/PII from errors and pad invite latency against email-enumeration timing attacks. Defence is in place; this is a **verify-on-release** item, not a defect.                 | The service-role key lives in these trusted Deno runtimes; a regression in redaction or the timing floor would open a side channel.                                                                    | In the release runbook, confirm: (1) edge-function logs never echo raw emails; (2) those logs aren't customer-visible; (3) the timing floor + jitter is still adequate. |
...
failure rolls back the data write); hard deletes are confined to the
Super-Admin danger zone (`super_admin_permanent_delete` + clean-slate/reset
helpers, all audit-paired and tombstoned); author-private Care Notes and the
...
| ID     | Sev | Location                                                                 | What                                                                                                                                                                                                                                   | Why it matters                                                                                                                                                                                                                                                                                         | Recommendation                                                                                                                                                                                                                                                                            |
| ------ | --- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TEST-1 | P1  | `tests/integration/**` + `.github/workflows/rls-integration.yml`         | RLS + action-pipeline integration tests run only **on schedule (Mon) / manual**, not on RLS-touching PRs (the workflow is path-filtered but non-gating).                                                                               | An RLS drift (e.g. an Over-Shepherd gaining sight of a sealed note) or a broken audit pairing can land **green on the PR** and hide for up to 7 days. The static fitness checks catch _new_ unaudited writes and _new_ classified tables, but **not a loosened `USING` clause on an existing policy**. | Make the integration job a **required check on PRs that touch** `supabase/migrations/**`, `lib/**/*reads*.ts`, `lib/**/*rpc*.ts`, `app/**/actions.ts`. The path filters already exist — promote to gating.                                                                                |
...
| DOC-5 | P1  | `CONTEXT.md:339`                                                                                                                  | The Admin OS entry says "Planning, **People**, Calendar, and Follow-ups tabs are hidden behind Super-Admin nav-visibility flags, default off."                                                                     | Contradicts ADR 0024, which defaults **People** (and Groups) nav ON (seed `20260701020000`). The authoritative glossary now misstates the shipped default. | Remove People from the "default off" list (keep Planning/Calendar/Follow-ups); confirm Groups is correctly described as default-on.                                                                            |
| DOC-6 | P2  | `docs/README.md:70`                                                                                                               | The ADR index says "0001–0025; the pivot is 0016–0022, amended by 0023–0024" but ADRs run to **0030**.                                                                                                             | Readers/agents under-count the decision catalog and miss 0026–0030.                                                                                        | Update to "0001–0030 …; later amendments 0026–0030 (flag reads, home workspace, page runner, manual Plan readiness, Multiply type-intent)".                                                                    |
...
- DOC-5 — `CONTEXT.md:339` marks People nav default-off (contradicts ADR 0024).
- TEST-1 — integration/RLS suite not gating on RLS-touching PRs (≤7-day blind spot).
- TEST-2 — no end-to-end coverage of the core Care/Plan/Multiply write flows.
- (DOC-6 is P2 but pairs with DOC-5 as the same "index/glossary lagged the pivot" theme.)

**P2 / Medium (11)** — SEC-1, SEC-2, SEC-3, ARCH-1, ARCH-2, ARCH-3, ARCH-4,
ARCH-5, ARCH-6, ARCH-8, DOC-3, DOC-6, TEST-3, TEST-4, TEST-5.

**P3 / Low (8)** — SEC-4, ARCH-7, DOC-7, DOC-8, TEST-6, TEST-7, TEST-8.

_(Counts in the executive summary group SEC/ARCH confirmations as "strong"
rather than findings; the index above lists only actionable rows.)_
