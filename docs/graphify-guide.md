...
becomes a *node*, and every "X calls Y" / "X imports Y" relationship becomes
an *edge*. The result is a map of the whole app (~6,600 nodes, ~15,700 edges)
...
> **Note:** GitHub's website will *not* render these pages — clicking
...
- **`graph.html` — the interactive graph.** The whole codebase as a zoomable
  ...
  details and connections. Colors are *communities* — clusters of code that
...
- you're making a *partial* commit (unstaged changes exist) — so the committed
...
| `npm run graph:rebuild` | After a refactor that *deleted* code — forces a full rebuild (`update --force`). |
...
- **Merge conflict on `graph.json`** — a custom merge driver usually handles
  this. If you hit one on a machine *without* graphify installed: take either
...
## What's deliberately *not* in the graph

Corpus scope is controlled by `.graphifyignore` — tooling directories,
secrets, and (until fixed upstream) `*.sh` scripts are excluded. See the
