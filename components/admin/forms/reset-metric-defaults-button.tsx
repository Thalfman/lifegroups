  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      >
        ...
        24-hour check-in due offset, etc.). Per-group overrides stay
        intact &mdash; clear those individually from the list below.
      </p>
      {state?.ok ? (
        <p style={successTextStyle}>Defaults restored.</p>
      ) : null}
      {state && !state.ok ? (
        <p style={errorTextStyle}>{state.errors[0]}</p>
      ) : null}
    </div>
  );
}
